...
class FileCommanderScreen(Screen, HelpableScreen, key_actions):
    ...
    def __init__(self, session, path_left=None):
        ...
        self.session = session
        Screen.__init__(self, session)
        # Debug: salva la skin in /tmp
        try:
            with open("/tmp/filecommander_skin_debug.xml", "w") as f:
                f.write(self.skin)
            print("[FileCommander] Skin salvata in /tmp/filecommander_skin_debug.xml")
        except Exception as e:
            print("[FileCommander] Errore salvataggio skin: %s" % str(e))
